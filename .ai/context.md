...
- Documentation updates (both `docs/` and `website/`)
...
- Create new branch (never push to main)
...
### Why Metadata Annotations?
- GitOps controllers manage pod templates
- Metadata is preserved by ArgoCD/Flux
- Webhook reads from metadata at pod creation
- No conflicts with GitOps sync

### Why Optional Webhook?
- Controller works standalone in Recommend mode
- Webhook enables Auto mode without template changes
...
- Supports authentication for production use

## Security Considerations

- Prometheus authentication support (v1.5.2+)
- RBAC for controller and webhook
- TLS for webhook (via cert-manager)
- No secrets in logs or annotations
- Audit trail via Kubernetes events

## Performance Characteristics

- Controller reconciliation: Configurable interval (default 5m)
- Webhook latency: <10ms typical
- Memory footprint: ~50MB controller, ~30MB webhook
- CPU usage: Minimal (<0.1 core typical)

## Known Limitations

- Requires Prometheus for metrics
- Webhook requires cert-manager
- No support for Jobs/CronJobs yet
- Single-cluster only (multi-cluster planned)
