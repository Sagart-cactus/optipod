...
echo "CPU Request: $(kubectl get deployment my-app -o jsonpath='{.metadata.annotations.optipod\.io/cpu-request\.nginx}')"
echo "Memory Request: $(kubectl get deployment my-app -o jsonpath='{.metadata.annotations.optipod\.io/memory-request\.nginx}')"
echo "CPU Limit: $(kubectl get deployment my-app -o jsonpath='{.metadata.annotations.optipod\.io/cpu-limit\.nginx}')"
echo "Memory Limit: $(kubectl get deployment my-app -o jsonpath='{.metadata.annotations.optipod\.io/memory-limit\.nginx}')"
```
...
kubectl get deployment my-app -o json | \
  jq -r '.metadata.annotations | to_entries |
    map(select(.key | startswith("optipod.io/cpu-request.") or startswith("optipod.io/memory-request."))) |
    group_by(.key | split(".")[1]) |
    map({container: .[0].key | split(".")[1], recommendations: map({(.key | split(".")[0] | split("/")[1]): .value}) | add})'
```
  ...
  template:
    spec:
      ...
      - name: nginx
        resources:
          requests:
            cpu: 250m      # From optipod.io/cpu-request.nginx
            memory: 512Mi  # From optipod.io/memory-request.nginx
          limits:
            cpu: 500m      # From optipod.io/cpu-limit.nginx
            memory: 1Gi    # From optipod.io/memory-limit.nginx
```
  ...
  jq -r '.items[] |
    select(.metadata.annotations."optipod.io/managed" == "true") |
    {
      ...
      containers: [
        ...
        map(select(.key | startswith("optipod.io/cpu-request."))) |
        map(.key | split(".")[1])
      ] | flatten | unique
    } |
...
kubectl get deployment $DEPLOYMENT -n $NAMESPACE -o json | \
  jq -r '
    ...
    to_entries |
    map(select(.key | startswith("optipod.io/cpu-request.") or
                      startswith("optipod.io/memory-request.") or
                      startswith("optipod.io/cpu-limit.") or
                      startswith("optipod.io/memory-limit."))) |
    group_by(.key | split(".")[1]) |
    map({
      container: .[0].key | split(".")[1],
      resources: (
        map({
          key: (.key | split(".")[0] | split("/")[1] | gsub("-"; "_")),
          value: .value
        }) | from_entries
      )
    }) |
