  ...
  template:
    ...
    spec:
      ...
      - name: nginx
        ...
        ports:
        - containerPort: 80
        # Optional: Set initial resources
        # OptiPod will optimize based on actual usage
        resources:
          requests:
            cpu: "100m"
            memory: "128Mi"
...
spec:
  updateStrategy:
    strategy: webhook  # Changed from ssa
    rolloutStrategy: onNextRestart
```
...
spec:
  updateStrategy:
    strategy: ssa  # Changed from webhook
    useServerSideApply: true
```

**Step 2: Configure ArgoCD (if needed)**

Add ignoreDifferences or upgrade to ArgoCD 2.5+.
...
- [Update Strategies](../concepts/update-strategies.md) - Understanding webhook vs SSA
